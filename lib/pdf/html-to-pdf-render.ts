...
export interface HtmlToPdfFooterOptions {
  /** Fixed app text, e.g. "Generated by PDFKit" — always drawn with jsPDF's own
   *  built-in font, which is fine since this string is never AI/user content. */
  text?: string;
...
export async function renderHtmlToPdf(
  ...
  try {
    ...
    if (options.footer?.text || options.footer?.showPageNumbers) {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        if (options.footer.text) {
          doc.text(options.footer.text, options.marginMm, pageHmm - options.marginMm / 2);
        }
        if (options.footer.showPageNumbers) {
          doc.text(`${i} / ${totalPages}`, pageWmm - options.marginMm, pageHmm - options.marginMm / 2, {
            align: "right",
          });
        }
      }
