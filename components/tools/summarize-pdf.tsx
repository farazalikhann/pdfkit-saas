"use client";

import * as React from "react";
import Link from "next/link";
import { toast } from "sonner";
import { UploadZone } from "@/components/tool-shell/upload-zone";
import { ActionBar, type ActionState } from "@/components/tool-shell/action-bar";
import { ProgressRing } from "@/components/tool-shell/progress-ring";
import { ServerSideNotice } from "@/components/tool-shell/client-badge";
import { OcrFallbackPrompt } from "@/components/tools/ocr-fallback-prompt";
import { Label } from "@/components/ui/label";
import { Button } from "@/components/ui/button";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import { useTextExtraction } from "@/lib/ai/use-text-extraction";
import { parseMarkdownLite, markdownBlocksToHtml, type MarkdownBlock } from "@/lib/ai/markdown-lite";
import type { SummaryLength } from "@/lib/ai/provider";
import { downloadBlob } from "@/lib/utils";
import { SITE_NAME } from "@/lib/constants";
import type { ToolDefinition } from "@/lib/tools";

const LIMITS = { maxPages: 50, maxCharacters: 100_000 };

function MarkdownBlockView({ block }: { block: MarkdownBlock }) {
  switch (block.type) {
    case "h1":
      return <h2 className="text-lg font-bold">{block.text}</h2>;
    case "h2":
      return <h3 className="text-base font-semibold">{block.text}</h3>;
    case "bullets":
      return (
        <ul className="list-inside list-disc space-y-1">
          {block.items.map((item, i) => (
            <li key={i}>{item}</li>
          ))}
        </ul>
      );
    case "tldr":
      return (
        <p className="rounded-lg border-l-4 border-muted-foreground/40 bg-muted/50 px-3 py-2">
          <strong>TLDR:</strong> {block.text}
        </p>
      );
    case "p":
    default:
      return <p>{block.text}</p>;
  }
}

export function SummarizePdfTool({ tool }: { tool: ToolDefinition }) {
  const [file, setFile] = React.useState<File | null>(null);
  const [length, setLength] = React.useState<SummaryLength>("bullets");

  const extraction = useTextExtraction(LIMITS);
  const [state, setState] = React.useState<ActionState>("idle");
  const [progress, setProgress] = React.useState(0);
  const [summary, setSummary] = React.useState<string | null>(null);
  const [errorMessage, setErrorMessage] = React.useState<string | null>(null);

  function handleFiles(files: File[]) {
    const f = files[0] ?? null;
    setFile(f);
    setSummary(null);
    setErrorMessage(null);
    setState("idle");
    extraction.reset();
  }

  async function runSummarize(text: string) {
    setState("processing");
    setProgress(0.9);
    setErrorMessage(null);
    try {
      const res = await fetch("/api/ai/summarize", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text, length }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Summarization failed");
      setSummary(data.summary);
      setProgress(1);
      setState("done");
    } catch (err) {
      console.error(err);
      setState("error");
      setErrorMessage(err instanceof Error ? err.message : "Something went wrong.");
      toast.error("Couldn't summarize this file", {
        description: err instanceof Error ? err.message : undefined,
      });
    }
  }

  async function handleSummarize() {
    if (!file) return;
    setErrorMessage(null);
    const text = await extraction.start(file);
    if (text) await runSummarize(text);
  }

  async function handleRunOcr() {
    const text = await extraction.runOcr();
    if (text) await runSummarize(text);
  }

  const isReading = extraction.phase === "extracting" || extraction.phase === "ocr-running";
  const isBusy = isReading || state === "processing";
  const busyLabel = isReading ? extraction.statusText : "Summarizing…";
  const busyProgress = isReading ? extraction.progress * 100 : progress * 100;
  const displayError =
    state === "error" ? errorMessage : extraction.phase === "error" ? extraction.error : null;

  function downloadTxt() {
    if (!summary) return;
    downloadBlob(new Blob([summary], { type: "text/plain" }), "summary.txt");
  }

  async function downloadPdf() {
    if (!summary) return;
    const { renderHtmlToPdf } = await import("@/lib/pdf/html-to-pdf-render");
    const blocks = parseMarkdownLite(summary);
    const html =
      `<div style="font-family:sans-serif;font-size:13px;line-height:1.6;color:#111;">` +
      `<h1 style="font-size:20px;margin:0 0 18px;">Summary</h1>` +
      markdownBlocksToHtml(blocks) +
      `</div>`;
    const bytes = await renderHtmlToPdf(html, {
      pageSize: "a4",
      orientation: "portrait",
      marginMm: 20,
      footer: { text: `Generated by ${SITE_NAME}`, showPageNumbers: true },
    });
    downloadBlob(new Blob([new Uint8Array(bytes)], { type: "application/pdf" }), "summary.pdf");
  }

  const blocks = summary ? parseMarkdownLite(summary) : [];

  return (
    <div className="mx-auto max-w-2xl px-4 pb-40 pt-4 md:pb-16">
      <div className="mb-4 space-y-2">
        <ServerSideNotice>
          This tool sends your document&apos;s text through OpenRouter to a third-party AI
          model provider for processing. See our{" "}
          <Link href="/privacy-policy" className="underline underline-offset-2">
            privacy policy
          </Link>
          .
        </ServerSideNotice>
      </div>

      {!file ? (
        <UploadZone accept={tool.accept} multiple={false} maxFiles={1} onFiles={handleFiles} acceptHint="One file at a time" />
      ) : isBusy ? (
        <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-border bg-muted/30 py-12">
          <ProgressRing progress={busyProgress} />
          <p className="text-sm text-muted-foreground">{busyLabel}</p>
        </div>
      ) : extraction.phase === "needs-ocr" ? (
        <OcrFallbackPrompt
          onRunOcr={handleRunOcr}
          onCancel={() => {
            setFile(null);
            extraction.reset();
          }}
        />
      ) : summary ? (
        <div className="space-y-4">
          <div className="space-y-3 rounded-xl border border-border bg-card p-4 text-sm leading-relaxed">
            {blocks.map((block, i) => (
              <MarkdownBlockView key={i} block={block} />
            ))}
          </div>
          <div className="flex gap-2">
            <Button variant="outline" className="flex-1" onClick={downloadTxt}>
              Download .txt
            </Button>
            <Button className="flex-1" onClick={downloadPdf}>
              Download PDF
            </Button>
          </div>
          <button
            type="button"
            onClick={() => {
              setFile(null);
              setSummary(null);
              setState("idle");
            }}
            className="mx-auto block text-sm font-medium text-muted-foreground underline-offset-4 hover:underline"
          >
            Summarize another file
          </button>
        </div>
      ) : (
        <div className="space-y-1.5">
          <Label>Summary length</Label>
          <Select value={length} onValueChange={(v) => setLength(v as SummaryLength)}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="short">Short (2-3 sentences)</SelectItem>
              <SelectItem value="bullets">Bullet points</SelectItem>
              <SelectItem value="detailed">Detailed</SelectItem>
            </SelectContent>
          </Select>
        </div>
      )}

      {displayError && (
        <p className="mt-3 rounded-lg bg-destructive/10 px-3 py-2 text-sm text-destructive">{displayError}</p>
      )}

      {state !== "done" && file && extraction.phase !== "needs-ocr" && !isBusy && (
        <ActionBar
          state={state === "idle" ? "ready" : state}
          label="Summarize with AI"
          progress={progress * 100}
          onAction={handleSummarize}
        />
      )}
    </div>
  );
}
